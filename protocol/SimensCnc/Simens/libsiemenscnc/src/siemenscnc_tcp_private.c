...
#include "../inc/siemenscnc_tcp_private.h"
#include "../inc/siemenscnc_private.h"

#ifdef OS_WIN32
static int _siemenscnc_tcp_init_win32(void)
{
...
static int _siemenscnc_tcp_set_ipv4_options(int s)
{
    int rc;
    int option;
...
/* Establishes a mc TCP connection with a Modbus server. */
int _siemenscnc_tcp_connect(siemenscnc_t *ctx)
{
    int rc;
    struct sockaddr_in addr;
    siemenscnc_tcp_t *ctx_tcp = ctx->backend_data;

#ifdef OS_WIN32
    if (_siemenscnc_tcp_init_win32() == -1) {
        return -1;
    }
#endif
    ...
    rc = _siemenscnc_tcp_set_ipv4_options(ctx->s);
    if (rc == -1) {
        close(ctx->s);
        ctx->s = -1;
        return -1;
...
int _siemenscnc_tcp_flush(siemenscnc_t *ctx)
{
    ...
    do {
        ...
        char devnull[SIEMENSCNC_MAX_MESSAGE_LENGTH];
#ifndef OS_WIN32
        rc = recv(ctx->s, devnull, SIEMENSCNC_MAX_MESSAGE_LENGTH, MSG_DONTWAIT);
#else
        ...
        if (rc == 1) {
            ...
            rc = recv(ctx->s, devnull, SIEMENSCNC_MAX_MESSAGE_LENGTH, 0);
        }
#endif
        if (rc > 0) {
            rc_sum += rc;
        }
    } while (rc == SIEMENSCNC_MAX_MESSAGE_LENGTH);
